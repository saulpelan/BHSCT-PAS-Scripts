...
'	Script name: Timeslot Maintenance Next
'	Author: Saul Pelan
'	Recommended map key: CTRL + =
'
'	┌Description────────────────────────────────────────────────────────────────────┐
'	│ Allows the maintenance user to jump to the next timeslot without having to 	│
...
'	│ have to revise the first one and jump to the next timeslot using this script	│
...
scriptName = "Timeslot Maintenance Next"
errorState = False
sleepMilliseconds = 1
test = False
...
Sub Main()
	...
	If title = "M a i n t a i n   D o c t o r   S e s s i o n" Or title = "D o c t o r   T e m p l a t e" Then
		If InStr(GetFunctionSubtitle, "Maintain Timeslot") Then
			...
			If GetText(8, 20, 8, 28) <> "REVISE" Then
				GoToField(2)
				Send("REVISE")
			End If
...
Sub SelectNextTimeslot()
	SendSpecial("VT_F14") 'Open superhelp list of timeslots
	SendSpecial("VT_CURSOR_DOWN") 'Select next timeslot
	Send("") 'send return to select timeslot from list
End Sub

Sub Db(ByVal text)
	If test Then
		MsgBox "** Timeslot Maintenance Next **" & VbCrLf & text
	End If
End Sub
