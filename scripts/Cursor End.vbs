...
'	Script name: Cursor End
'	Author: Saul Pelan
'
'	┌Description────────────────────────────────────────────────────────────────────┐
'	│ This script attempts to send the terminal cursor to the end of the current	│
'	│ field, simulating the "End" keyboard key.					│
'	└───────────────────────────────────────────────────────────────────────────────┘
'
'	┌Instructions───────────────────────────────────────────────────────────────────┐
'	│ In CRT, map a key (End key recommended) to this script. Press the mapped key	│
'	│ when editing a field on CLINiCOM.						│
'       └───────────────────────────────────────────────────────────────────────────────┘

scriptName = "Cursor End"
...
Sub Main()
	...
	If WaitForCursorTimeout(A0) Then
		...
		' Check to see if the cursor is now to the left of its original position, and if it is, send the 
		' cursor position modifying character again in order to send .
		If newcol < col Then
			crt.Screen.SendSpecial("VT_SELECT")
		End If	
	End If
...
' after the amuont of milliseconds specified in option A0
